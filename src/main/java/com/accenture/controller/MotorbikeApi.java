...
public interface MotorbikeApi {
    ...
    @GetMapping("/{id}")
    ResponseEntity<MotorbikeResponseDto> getById(@Parameter(description = "Motorbike ID", required = true) @PathVariable("id") int id);
    ...
    @ApiResponse(responseCode = "404", description = "Motorbike not found",
            content = @Content(schema = @Schema(implementation = ErrorDto.class)))
    @DeleteMapping("/{id}")
    ResponseEntity<Void> delete(@Parameter(description = "Motorbike ID", required = true) @PathVariable("id") int id);
    ...
    @ApiResponse(responseCode = "404", description = "Motorbike not found",
            content = @Content(schema = @Schema(implementation = ErrorDto.class)))
    @PutMapping("/{id}")
    ResponseEntity<MotorbikeResponseDto> put(@Parameter(description = "ID of motorbike", required = true)@PathVariable int id, @RequestBody MotorbikeRequestDto requestDto);
    ...
    @ApiResponse(responseCode = "404", description = "Motorbike not found",
            content = @Content(schema = @Schema(implementation = ErrorDto.class)))
    @PatchMapping("/{id}")
    ResponseEntity<MotorbikeResponseDto> patch(@Parameter(description = "ID of motorbike", required = true)@PathVariable int id,@RequestBody MotorbikeRequestDto requestDto);
}
